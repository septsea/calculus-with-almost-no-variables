...
\begin{definition}
    设 $I$ 为区间. 设 $f$ 为 $I$ 上的函数. 若存在 $I$ 上的可导函数 $F$ 使 $\mathrm{D}F = f$, 则说 $F$ 是 $f$ 的一个\emph{原函数}\index{yuan2han2shu4@原函数}.
\end{definition}

\begin{definition}
    设 $I$ 为区间. 设 $f$ 为 $I$ 上的函数. 设 $f$ 有一个原函数. 那么, 称 $f$ 的\emph{全体}原函数作成的\emph{集}为 $f$ 的\emph{不定积分}\index{bu4ding4ji1fen1@不定积分}, 即
    \begin{align*}
...
\begin{definition}
    设 $A$ 为集. 若任取 $A$ 的元 $f$, $f$ 是一个函数, 就称 $A$ 为\emph{函数集}\index{han2shu4ji1@函数集}.
...
\begin{example}
    ...
    \begin{itemize}
        \item $p + 0 = p$;
        \item 存在 $P$ 上的函数 $q$, 使 $p + q = 0$ (取 $q$ 为 $-p$ 即可).
    \end{itemize}
...
\begin{definition}
    设 $I$ 是区间. 设 $A$ 是 $I$ 上的函数集, 且 $A$ 的每个元都是可导函数. 我们说, $A$ 是 $I$ 上的\emph{可导函数集}\index{ke3dao3han2shu4ji2@可导函数集}. 定义
    \begin{align*}
...
\begin{definition}
    设 $P \subset \mathbb{R}$. 设 $A$ 为 $P$ 上的函数集. 若存在 $a \in A$ 使 $P = \{ a \} + \mathbb{R} = a + \mathbb{R}$, 就说 $A$ 是 $P$ 上的\emph{至多相差常函数的函数集}\index{zhi4duo1xiang1cha1chang2han2shu4de5han2shu4ji2@至多相差常函数的函数集}.
\end{definition}
